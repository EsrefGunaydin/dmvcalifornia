...
7. **Generate the hero image via Pexels.** From the repo root, run:

   ```
   npm install --no-audit --no-fund
   PEXELS_API_KEY=<key from routine config> node scripts/backfill-blog-images.js --slug=<your-new-slug>
   ```

   The `PEXELS_API_KEY` is provided inline in the routine prompt — read it from there. The script reads the post's title + primary tag, fetches a unique landscape photo from Pexels, downloads it, crops to 1200×630, converts to WebP, saves to `public/images/blog/blog-<slug>.webp`, and sets the post's `hero_image` + `hero_image_credit` fields automatically. It de-dupes against every other post's `photo_id` so no two posts share a photo.

   **Security:** the key is for transient use only — never write it into any committed file (no `.env`, no script, no doc, no log).
